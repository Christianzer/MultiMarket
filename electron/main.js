import { app as i, BrowserWindow as c, ipcMain as s, shell as g } from "electron";
...
function m() {
  ...
  }), e.webContents.session.setSpellCheckerEnabled(!1), e.webContents.session.setPreloads([]), d ? (e.loadURL(d), process.env.NODE_ENV === "development" && e.webContents.openDevTools()) : e.loadFile(r.join(p, "index.html")), e.webContents.setWindowOpenHandler(({ url: o }) => (o.startsWith("https:") && g.openExternal(o), { action: "deny" })), e.on("minimize", () => {
  }), e.on("closed", () => {
    e = null;
...
function v() {
  process.env.NODE_ENV === "production" && !process.env.VITE_DEV_SERVER_URL ? (console.log("Setting up auto-updater for production..."), t.logger = console, t.checkForUpdatesAndNotify = !0, t.on("checking-for-update", () => {
    ...
    t.checkForUpdatesAndNotify().catch((o) => {
...
s.handle("print-receipt", async (o, a) => {
  try {
    ...
    const l = await n.webContents.print(f);
    return console.log("Impression:", l ? "Réussie" : "Annulée par l'utilisateur"), setTimeout(() => {
      n.isDestroyed() || n.close();
    }, 1e3), {
      success: !0,
      printed: l,
      message: l ? "Document imprimé avec succès" : "Impression annulée par l'utilisateur"
    };
  } catch (n) {
...
s.handle("check-for-updates", async () => {
  try {
    return process.env.NODE_ENV === "production" && !process.env.VITE_DEV_SERVER_URL ? (console.log("Manual update check requested"), { success: !0, result: await t.checkForUpdatesAndNotify() }) : { success: !1, error: "Auto-updater is disabled in development mode" };
  } catch (o) {
...
s.handle("get-app-version", () => i.getVersion());
